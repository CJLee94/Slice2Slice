...
class OCT_Repeat_Dataset(Dataset):
    def __init__(self, 
                 fpath,
                 fpath2=None,
                 augment_params=dict(),
        ...
        # pdb.set_trace()
        if type(fpath) is list:
            for fidx, fp in enumerate(fpath):
                ...
                if self.fpath2:
                    im2 = np.asanyarray(nib.load(self.fpath2[fidx]).dataobj)
                    if slice_direction == "enface":
        ...
        if self.fpath2:
            self.img2 = img2.astype(np.float32) / self.scale_factor - 0.5
...
def parse_args():
    ...
    parser.add_argument("-r", "--data_fpath2", default=None, nargs="+", help="aligned_repeat_scan")
    ...
    parser.add_argument("--direction", default="Ascan", type=str, help="Slice direction, can be Ascan or enface")
...
def main():
    ...
    for df in args.data_fpath:
        if os.path.isdir(df):
            train_flist += sorted(glob.glob(os.path.join(df, "*")))
        else:
            train_flist.append(df)

    if args.data_fpath2:
        for df in args.data_fpath2:
            if os.path.isdir(df):
                train_flist2 += sorted(glob.glob(os.path.join(df, "*")))
            else:
                train_flist2.append(df)
    ...
    dataset = OCT_Repeat_Dataset(train_flist, fpath2=train_flist2, slice_direction=args.direction)
    testset = OCT_Repeat_Dataset(test_flist, mode="test", slice_direction=args.direction)

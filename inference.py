...
def main():
    ...
    testset = OCT_Dataset(args.test_data_fpath, mode="test")
    ...
    with tqdm(total=len(testloader)) as pbar:
        ...
        for step, (input_images, target_images) in enumerate(testloader):
            ...
            if step%6==0:
                ...
                plt.imsave(os.path.join(args.save_dir, 'img{}.png'.format(step)), img, vmin=0.0, vmax=1.0, cmap="gray")
        out = np.stack(out)
        nib.save(nib.Nifti1Image(out.astype(np.uint8), np.eye(4)), os.path.join(args.save_dir, "Dn_"+img_basename+".nii"))
